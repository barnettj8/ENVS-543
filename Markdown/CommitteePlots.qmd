---
title: "Thesis Plots (6/15)- Final Versions"
...
ggsave("combined_plots_600dpi.png", combined_plot,
...
p1 <- ggplot(meanthesis, aes(x = `TSS (mg/L)`, y = `kd (m-1)`)) +
  ...
  theme_minimal() +
  theme(
    plot.title = element_text(size = 8, color = "black"),  # Chart title black
     axis.line = element_line(colour = "black", size = 0.5),  # Add black axis lines
    axis.title.x = element_text(size = 9, color = "black"),  # X-axis title black
    axis.title.y = element_text(size = 9, color = "black")   # Y-axis title black
  )
...
p2 <- ggplot(meanthesis, aes(x = `CDOM a440 (corr)`, y = `Residual kd after TSS`)) +
  geom_point(color = "black") +  # Use a lighter gray for points
  geom_smooth(method = "lm", color = "black", se = FALSE) +  # Black regression line
  labs(title = " ", x = "CDOM (m⁻¹)", y = "Residual kd after TSS (m⁻¹)") +
...
p3 <- ggplot(meanthesis, aes(x = `CHLa (ug/L)`, y = `kd (m-1)`)) +
  geom_point(color = "black") +  # Use an even lighter gray for points
  geom_smooth(method = "lm", color = "black", se = FALSE) +  # Black regression line
  labs(title = " ", x = "CHLa (µg L⁻¹)", y = "kd (m⁻¹)") +
...
p1 <- ggplot(meanthesis, aes(x = `TSS (mg/L)`, y = `kd (m-1)`)) +
  ...
  geom_text(x = max(meanthesis$`TSS (mg/L)`), y = max(meanthesis$`kd (m-1)`), 
            label = paste(reg_stats_p1$eq, "\n", reg_stats_p1$rsq, "\n", reg_stats_p1$pval, "\n", reg_stats_p1$se), 
            hjust = 1, vjust = 2, size = 3, color = "black") +  # Text color black
  theme_minimal() +
  theme(
    plot.title = element_text(size = 8, color = "black"),  # Chart title black
     axis.line = element_line(colour = "black", size = 0.5),  # Add black axis lines
...
p3 <- ggplot(meanthesis, aes(x = `CHLa (ug/L)`, y = `kd (m-1)`)) +
  ...
  geom_text(x = max(meanthesis$`CHLa (ug/L)`), y = max(meanthesis$`kd (m-1)`), 
            label = paste(reg_stats_p3$eq, "\n", reg_stats_p3$rsq, "\n", reg_stats_p3$pval, "\n", reg_stats_p3$se), 
            hjust = 1, vjust = 3, size = 3, color = "black") +  # Text color black
  theme_minimal() +
  theme(
    plot.title = element_text(size = 8, color = "black"),  # Chart title black
     axis.line = element_line(colour = "black", size = 0.5),  # Add black axis lines
...
# Display plots in a grid
plotprint <- grid.arrange(p1, p2, p3, ncol = 1,
             heights = c(1,1,1))

print(plotprint)

#ggsave("figure3option.png", plotprint,
...
get_regression_stats <- function(x, y) {
  model <- lm(y ~ x)
  eq <- paste("y = ", round(coef(model)[2], 2), "x + ", round(coef(model)[1], 2), sep = "")
  rsq <- paste("R² = ", round(summary(model)$r.squared, 2), sep = "")
  pval <- paste("p = ", round(summary(model)$coefficients[2, 4], 3), sep = "")
  se <- paste("SE = ", round(summary(model)$coefficients[2, 2], 3), sep = "")  # Extract SE
  return(list(eq = eq, rsq = rsq, pval = pval, se = se))
}
...
p1 <- ggplot(meanthesis, aes(x = `TSS (mg/L)`, y = `kd (m-1)`)) +
  ...
  labs(title = " ", x = "TSS (mg L⁻¹)", y = "kd (m⁻¹)") +
  geom_text(x = max(meanthesis$`TSS (mg/L)`), y = max(meanthesis$`kd (m-1)`), 
            label = paste(reg_stats_p1$rsq, "\n", reg_stats_p1$pval), 
            hjust = 1, vjust = 4, size = 3, color = "black") +  # Text color black
  theme_minimal() +
  theme(
    plot.title = element_text(size = 8, color = "black"),  # Chart title black
     axis.line = element_line(colour = "black", size = 0.5),  # Add black axis lines
...
#ggsave("figure3option2.png", plotprint,
 #      width = 5, height = 8, dpi = 600, units = "in")
...
meanthesis <- meanthesis %>%
  mutate(Lake_Size = case_when(
    `Lake` == "Briery Creek Lake" ~ 845,
    `Lake` == "Chickahominy Lake" ~ 1230,
    `Lake` == "Harrison Lake" ~ 82,
    `Lake` == "Fort Barfoot Reservoir" ~ 384,
    `Lake` == "Lake Anna" ~ 13000,
    `Lake` == "Sandy River Reservoir" ~ 740,
    `Lake` == "Swift Creek Reservoir" ~ 1700,
    TRUE ~ NA_real_  # Assign NA if none match
  ))

mean_by_lake <- meanthesis %>%
  group_by(`Lake`) %>%
  summarize(across(where(is.numeric), ~ mean(.x, na.rm = TRUE), .names = "mean_{.col}"))
...
model <- lm(`mean_kd (m-1)` ~ mean_Lake_Size, data = mean_by_lake)
summary(model)
...
ggplot(mean_by_lake, aes(x = mean_Lake_Size, y = `mean_kd (m-1)`)) +
  geom_point(size = 3) +
  geom_smooth(method = "lm", se = TRUE, color = "blue") +
  labs(
    x = "Mean Lake Size (acres)",
    y = "Mean kd (m⁻¹)",
    title = "Regression of Mean kd vs Lake Size"
  ) +
  theme_minimal()
...
get_regression_stats_se <- function(x, y) {
  ...
  eq <- paste("y = ", round(coef_values[2], 3), "x + ", round(coef_values[1], 3), sep = "")
  ...
  residual_se <- paste("SE = ", round(summary(model)$sigma, 3), sep = "")
...
annadata$residualchla <- annadata$kd - (annadata$Photic_CHLa * 0.07)

# p2 - Regression: Photic_CDOM vs residualchla
reg_stats_p2 <- get_regression_stats_se(annadata$Photic_CDOM, annadata$residualchla)
p2 <- ggplot(annadata, aes(x = Photic_CDOM, y = residualchla)) +
  geom_point(color = "gray70") +  # Use a lighter gray for points
  geom_smooth(method = "lm", color = "black", se = FALSE) +  # Black regression line
  labs(title = "Lake Anna: Photic Zone CDOM vs residualchla", x = "CDOM (m⁻¹)", y = "Residual CHLa") +
  geom_text(x = max(annadata$Photic_CDOM, na.rm = TRUE), y = max(annadata$residualchla, na.rm = TRUE), 
            label = paste(reg_stats_p2$eq, "\n", reg_stats_p2$rsq, "\n", reg_stats_p2$pval, "\n", reg_stats_p2$residual_se), 
            hjust = 1, vjust = 1, size = 2, color = "black") +  # Text color black
  theme_minimal() +
  theme(
...
# Plot p2 with residualchla on the y-axis
print(p2)
...
lm(kd ~ Photic_TSS + Photic_CDOM + Photic_CHLa, data = annadata)
...
lm(kd ~ Photic_TSS + Photic_CDOM + Photic_CHLa, data = bukanna)
...
```
Model selection based on Akaike’s Information Criterion (AIC) indicated that Photic_CHLa alone was the best predictor of light attenuation (kd), with the lowest AIC value (–36.02). Models that included CHLa with either TSS or CDOM showed slightly higher AIC values (ΔAIC < 2), suggesting they are also plausible but do not substantially improve model performance. In contrast, models without CHLa—including those with only TSS, only CDOM, or both—had significantly higher AIC values (ΔAIC > 4), indicating weaker support. Overall, these results suggest that CHLa is the most parsimonious and informative predictor of kd in this dataset, with little gain in explanatory power from adding other variables.
...
et_regression_stats <- function(x, y) {
  ...
  data <- na.omit(data.frame(x, y))
  model <- lm(y ~ x, data = data)
  eq <- paste("y = ", round(coef(model)[2], 3), "x + ", round(coef(model)[1], 3), sep = "")
  rsq <- paste("R² = ", round(summary(model)$r.squared, 3), sep = "")
  pval <- paste("P = ", round(summary(model)$coefficients[2, 4], 3), sep = "")
    se <- paste("SE = ", round(summary(model)$coefficients[2, 2], 3), sep = "")  # Extract SE
  return(list(eq = eq, rsq = rsq, pval = pval, se = se))
...
p1 <- ggplot(meanthesis, aes(x = `Secchi (m)`, y = `kd (m-1)`)) +
  ...
  theme(
    plot.title = element_text(size = 12, color = "black"),  # Title size and color
     axis.line = element_line(colour = "black", size = 0.5),  # Add black axis lines
    axis.title.x = element_text(size = 12, color = "black"),  # X-axis title size and color
    axis.title.y = element_text(size = 12, color = "black"),  # Y-axis title size and color
    axis.text.x = element_text(size = 12, hjust = 1),
    axis.text.y = element_text(size = 12)# Rotate x-axis labels for readability
  )
...
t <- ggplot(site_averages, aes(x = avg_TSS, y = avg_POC)) +
  geom_point(color = "black") +  # Use a darker gray for points
  geom_smooth(method = "lm", color = "black", se = FALSE) +  # Black regression line
  labs(title = " ", x = "TSS (mg L⁻¹)", y = "POC (mgC L⁻¹)") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 12, color = "black"),  # Title size and color
     axis.line = element_line(colour = "black", size = 0.5),  # Add black axis lines
...
c <- ggplot(site_averages, aes(x = avg_CHLA, y = avg_POC)) +
  geom_point(color = "black") +  # Use a darker gray for points
  geom_smooth(method = "lm", color = "black", se = FALSE) +  # Black regression line
  labs(title = " ", x = "CHLa (ug L⁻¹)", y = "POC (mgC L⁻¹)") +
  theme_minimal() +
  theme(
    plot.title = element_text(size = 12, color = "black"),  # Title size and color
     axis.line = element_line(colour = "black", size = 0.5),  # Add black axis lines
...
avg_cpercent <- pocdata %>%
  filter(!is.na(Site)) %>%   # Remove NA sites first
  group_by(Site) %>%
  summarize(
    Average_Cpercent = mean(`C%`, na.rm = TRUE),
    SE_Cpercent = sd(`C%`, na.rm = TRUE) / sqrt(n())
  ) %>%
...
site_order <- c("Harrison", "Chickahominy", "BRI_1", "BRI_2", "SDY_1", "SDY_2", "SDY_3", 

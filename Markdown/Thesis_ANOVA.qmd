...
# Create regression equation text
reg_text <- paste0("y = ", intercept, " + ", slope, " * x\n",
                   "R² = ", r_squared, ", p = ", p_value)
...
ggplot(mean_values, aes(x = mean_CDOM_ab, y = mean_kd)) +
  ...
  geom_smooth(method = "lm", color = "red", se = TRUE) +
  labs(
    title = "Regression of Mean kd vs Mean CDOM_ab",
    x = "Mean CDOM_ab",
    y = "Mean kd"
  ) +
  annotate("text", x = max(mean_values$mean_CDOM_ab), 
           y = min(mean_values$mean_kd), 
           label = reg_text, hjust = 1, size = 4, color = "darkred") +
  theme_minimal()
